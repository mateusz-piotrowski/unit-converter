...
val namesToMeasures = measures
    .flatMap { m ->
        m.allNames.map { name -> name.lowercase() to m }
    }.toMap()
...
fun solveAuthors(sin: Scanner, sout: PrintStream) {
    while (true) {
        ...
        fun readMeasure(): String {
            var word = sin.next()
            if (word.lowercase() == "degree" || word.lowercase() == "degrees") {
                word += " " + sin.next()
            }
            return word
        }
        ...
        val m1 = namesToMeasures[m1Str.lowercase()]
        val m2 = namesToMeasures[m2Str.lowercase()]
